...
class OCSA:
    ...
    def SetGraphType(self, Number, Type, Polar=0):
        ...
        if not self.__Simulation:
            Command = (
                "CSPGRAPHTYPE" + str(Number) + "," + str(Type) + "," + str(Polar) + "\n"
            )
            Send(self.__Connexion, Command)
    ...
    def GetSpectrum(self, XScale="nm", YScale="log", Polar="1+2", TraceNumber=1):
        ...
        if not self.__Simulation:
            ...
            if XScale.lower() == "nm":
                Command = (
                    "CSPSPECTRWL" + str(int(TraceNumber)) + "," + str(Polar) + "\n"
                )
            else:
    ...
    def SaveToFile(self, FileName, TraceNumber=1, Type="spectrum"):
        ...
        if not self.__Simulation:
            Command = (
                "CSPSAVE" + str(Type) + str(TraceNumber) + "_" + str(FileName) + "\n"
            )
            Send(self.__Connexion, Command)
    ...
    def LoadFromFile(self, FileName):
        ...
        """
        from .Constantes import APXXXX_ERROR_BAD_FILENAME
        from .Errors import ApexError
        from os.path import isdir, dirname
