from .Common import Send, Receive, ReceiveUntilChar, recvall
import sys
import numpy as np
import struct
...
class OSA:
    ...
    def SetNPoints(self, NPoints):
        ...
        from .Constantes import AP2XXX_MINNPTS, AP2XXX_MAXNPTS
        from .Errors import ApexError
    ...
    def GetData(self, ScaleX="nm", ScaleY="log", TraceNumber=1):
        ...
        from .Constantes import (
            SimuAP2XXX_StartWavelength,
            SimuAP2XXX_StopWavelength,
        )
    ...
    def GetDataBin(self, ScaleX="nm", ScaleY="log", TraceNumber=1):
        ...
        if not self.__Simulation:
            ...
            YData = np.array(
                struct.unpack("f" * self.__NPoints, YData), dtype=np.float32
            )
            ...
            XData = np.array(
                struct.unpack("d" * self.__NPoints, XData), dtype=np.double
            )
